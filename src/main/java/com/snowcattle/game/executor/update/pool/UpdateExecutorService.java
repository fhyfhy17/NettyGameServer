...
/**
 * Created by jiangwenping on 17/1/11. 更新执行器
 */
public class UpdateExecutorService implements IUpdateExecutor {
	...
	public UpdateExecutorService(int corePoolSize, int maxSize, RejectedPolicyType rejectedPolicyType) {
		...
		nonOrderedQueuePoolExecutor = new NonOrderedQueuePoolExecutor(Constants.Thread.UpdateExecutorService,
				corePoolSize, maxSize, gameThreadPoolHelpFactory.createPolicy(rejectedPolicyType, name));
	}
	...
	public void executorUpdate(DispatchThread dispatchThread, IUpdate iUpdate, boolean firstFlag,
		...
		LockSupportUpdateFutureThread lockSupportUpdateFutureThread = new LockSupportUpdateFutureThread(dispatchThread,
				iUpdate, lockSupportUpdateFuture);
	...
	public void shutdown() {
		ExecutorUtil.shutdownAndAwaitTermination(nonOrderedQueuePoolExecutor, 60, TimeUnit.MILLISECONDS);
	}
}
